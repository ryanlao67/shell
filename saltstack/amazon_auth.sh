...
#Join in domain.
sudo yum -y install sssd realmd krb5-workstation samba-common-tools &> /dev/null
...
if [ -z "$N_KEY" ];then
sed  -i /password/a"session    required    pam_mkhomedir.so skel=/etc/skel/ umask=0022" $PAM_FILE
fi

#install SSM
	rpm -q amazon-ssm-agent &> /dev/null ||sudo yum install -y https://s3.amazonaws.com/ec2-downloads-windows/SSMAgent/latest/linux_amd64/amazon-ssm-agent.rpm
